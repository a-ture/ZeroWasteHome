...
class GestioneListaSpesaServiceTest {
    ...
    @InjectMocks
    private GestioneListaSpesaService gestioneListaSpesaService;
    ...
    void test_TC_GPL_GL_01() {
        ...
        List<PossiedeInFrigo> possiedeInFrigo = List.of(
                createPossiedeInFrigo("Latte", "senza-glutine"),
                createPossiedeInFrigo("Pane", "vegano")
        );

        List<Prodotto> pianoGiornaliero = List.of(
    ...
    void test_TC_GPL_GL_02() {
        ...
        assertEquals(1, result.getProducts().size(), "La lista della spesa dovrebbe contenere un solo prodotto");
        assertEquals("Uova", result.getProducts().get(0).getName(), "Il prodotto nella lista della spesa dovrebbe essere 'Uova'");
    }
    ...
    void testIsExpiring() {
        ...
        assertTrue(gestioneListaSpesaService.isExpiring(today.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")), today));
        // Caso: Data di scadenza nei prossimi 2 giorni
        assertTrue(gestioneListaSpesaService.isExpiring(today.plusDays(2).format(DateTimeFormatter.ofPattern("dd/MM/yyyy")), today));
        // Caso: Data di scadenza passata
        assertFalse(gestioneListaSpesaService.isExpiring(today.minusDays(1).format(DateTimeFormatter.ofPattern("dd/MM/yyyy")), today));
        // Caso: Data di scadenza troppo futura
        assertFalse(gestioneListaSpesaService.isExpiring(today.plusDays(3).format(DateTimeFormatter.ofPattern("dd/MM/yyyy")), today));
    ...
    void testIsCompatibleWithPreferences() {
        ...
        // Caso: Compatibile
        assertTrue(gestioneListaSpesaService.isCompatibleWithPreferences(List.of("vegano"), preferences));

        // Caso: Non compatibile
        assertFalse(gestioneListaSpesaService.isCompatibleWithPreferences(List.of("carne"), preferences));

        // Caso: Nessuna preferenza impostata
        assertTrue(gestioneListaSpesaService.isCompatibleWithPreferences(List.of("vegano"), Collections.emptyList()));

        // Caso: Prodotto senza categorie
        assertFalse(gestioneListaSpesaService.isCompatibleWithPreferences(Collections.emptyList(), preferences));
    }
    ...
    void testGenerateShoppingList_ExpiringProducts() {
        ...
        List<PossiedeInFrigo> possiedeInFrigo = List.of(
                new PossiedeInFrigo(utente, createProdotto("Latte", "senza-glutine"), 1, today.plusDays(1).format(DateTimeFormatter.ofPattern("dd/MM/yyyy"))), // In scadenza
                new PossiedeInFrigo(utente, createProdotto("Pane", "vegano"), 1, today.plusDays(10).format(DateTimeFormatter.ofPattern("dd/MM/yyyy"))) // Non in scadenza
        );
        ...
        assertNotNull(result, "La lista della spesa non dovrebbe essere null");
        assertEquals(1, result.getProducts().size(), "La lista della spesa dovrebbe contenere solo i prodotti in scadenza");
        assertEquals("Latte", result.getProducts().get(0).getName(), "Il prodotto nella lista della spesa dovrebbe essere 'Latte'");
    }

    private PossiedeInFrigo createPossiedeInFrigo(String nomeProdotto, String categoria) {
        Prodotto prodotto = new Prodotto(nomeProdotto, "123", categoria != null ? List.of(categoria) : List.of());
